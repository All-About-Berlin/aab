...
ctx["SOLIDARITY_TAX_MILDERUNGSZONE_MIN_INCOME_TAX"] = fail_on(
    "2026-12-31", int(_v["SOLIDARITY_TAX_MILDERUNGSZONE_MIN_INCOME_TAX"])
)  # § 3 SolzG [SOLZFREI]
...
ctx["VORSORGEPAUSCHAL_MIN"] = fail_on("2026-12-31", int(_v["VORSORGEPAUSCHAL_MIN"]))  # § 39b Abs. 2.3.e EStG
ctx["VORSORGEPAUSCHAL_MIN_TAX_CLASS_3"] = int(_v["VORSORGEPAUSCHAL_MIN_TAX_CLASS_3"])  # ??
ctx["ARBEITNEHMERPAUSCHALE"] = fail_on("2026-12-31", int(_v["ARBEITNEHMERPAUSCHALE"]))  # (€/y) - § 9a EStG
ctx["SONDERAUSGABEN_PAUSCHBETRAG"] = fail_on(
    "2026-03-20", int(_v["SONDERAUSGABEN_PAUSCHBETRAG"])
)  # (€/y) § 10c EStG [SAP]
...
ctx["KINDERGELD"] = fail_on("2026-12-31", int(_v["KINDERGELD"]))

# Tax break for parents (€/y) - § 32 Abs. 6 EStG [KFB] - monitored
ctx["KINDERFREIBETRAG"] = fail_on("2026-03-20", int(_v["KINDERFREIBETRAG"]))

# Tax break for single parents (€/y) - § 24b EStG [EFA]
ctx["ENTLASTUNGSBETRAG_ALLEINERZIEHENDE"] = fail_on("2026-03-20", int(_v["ENTLASTUNGSBETRAG_ALLEINERZIEHENDE"]))
ctx["ENTLASTUNGSBETRAG_ALLEINERZIEHENDE_EXTRA_CHILD"] = fail_on(
    "2026-03-20", int(_v["ENTLASTUNGSBETRAG_ALLEINERZIEHENDE_EXTRA_CHILD"])
)

ctx["CAPITAL_GAINS_TAX_RATE"] = Decimal(_v["CAPITAL_GAINS_TAX_RATE"])  # (%) - § 32d EStG
ctx["CAPITAL_GAINS_FREIBETRAG"] = int(_v["CAPITAL_GAINS_FREIBETRAG"])  # Sparer-Pauschbetrag, § 20 Abs. 9 EStG

# Below that amount (€/y), you don't pay Gewerbesteuer - § 11 GewStG
ctx["GEWERBESTEUER_FREIBETRAG"] = int(_v["GEWERBESTEUER_FREIBETRAG"])
...
ctx["KLEINUNTERNEHMER_MAX_INCOME_FIRST_YEAR"] = int(_v["KLEINUNTERNEHMER_MAX_INCOME_FIRST_YEAR"])  # § 19 Abs. 1 UStG
ctx["KLEINUNTERNEHMER_MAX_INCOME"] = int(_v["KLEINUNTERNEHMER_MAX_INCOME"])  # § 19 Abs. 1 UStG

# Above that amount (€/y), you must use double entry bookkeeping - § 241a HGB
ctx["DOUBLE_ENTRY_MIN_REVENUE"] = int(_v["DOUBLE_ENTRY_MIN_REVENUE"])
ctx["DOUBLE_ENTRY_MIN_INCOME"] = int(_v["DOUBLE_ENTRY_MIN_INCOME"])
...
ctx["EU_VAT_SCHWELLENWERT"] = int(_v["EU_VAT_SCHWELLENWERT"])

# Umsatzsteuer-Voranmeldung minimum amounts, based on VAT paid last year (€/year) - § 18 UStG
ctx["VAT_MIN_QUARTERLY_AMOUNT"] = int(_v["VAT_MIN_QUARTERLY_AMOUNT"])
ctx["VAT_MIN_MONTHLY_AMOUNT"] = int(_v["VAT_MIN_MONTHLY_AMOUNT"])
...
ctx["MIDIJOB_MAX_INCOME"] = fail_on("2026-03-20", int(_v["MIDIJOB_MAX_INCOME"]))
...
ctx["GKV_AZUBI_FREIBETRAG"] = fail_on("2026-12-31", int(_v["GKV_AZUBI_FREIBETRAG"]))  # § 20 Abs. 3 SGB IV
...
ctx["GKV_FREIWILLIG_VERSICHERT_MIN_INCOME"] = fail_on(
    "2026-12-31", int(_v["GKV_FREIWILLIG_VERSICHERT_MIN_INCOME_MONTHLY"]) * 12
)

# If you earn less than that (€/y), private health insurers usually reject you
ctx["PKV_MIN_INCOME"] = fail_on("2026-12-31", int(_v["PKV_MIN_INCOME"]))
...
ctx["TRAVEL_INSURANCE_COST"] = fail_on("2026-12-31", int(_v["TRAVEL_INSURANCE_COST"]))  # Guesstimated
ctx["EXPAT_INSURANCE_COST"] = fail_on(
    "2026-03-20",
    {
        "feather-basic": int(_v["EXPAT_INSURANCE_COST_FEATHER_BASIC"]),
        "feather-premium": int(_v["EXPAT_INSURANCE_COST_FEATHER_PREMIUM"]),
        "ottonova-expat": int(_v["EXPAT_INSURANCE_COST_OTTONOVA_EXPAT"]),
        "hansemerkur-basic": int(_v["EXPAT_INSURANCE_COST_HANSEMERKUR_BASIC"]),
        "hansemerkur-profi": int(_v["EXPAT_INSURANCE_COST_HANSEMERKUR_PROFI"]),
    },
)

ctx["EXPAT_STUDENT_COST"] = fail_on("2026-03-20", int(_v["EXPAT_STUDENT_COST"]))  # /out/feather-expats

# Maximum daily Krankengeld
ctx["GKV_KRANKENGELD_DAILY_LIMIT"] = (ctx["GKV_MAX_INCOME"] * Decimal("0.7") / 360).normalize()  # § 47 SGB V

# BAFöG Bedarfssatz (€/y)
ctx["BAFOG_BEDARFSSATZ"] = fail_on("2026-06-01", int(_v["BAFOG_BEDARFSSATZ"]))  # § 13 BAföG Abs 1.2 + 2.2
ctx["SPERRKONTO_AMOUNT"] = fail_on(
    "2026-06-01",
    (ctx["BAFOG_BEDARFSSATZ"] + int(_v["SPERRKONTO_SURCHARGE_INSURANCE"]) + int(_v["SPERRKONTO_SURCHARGE_OTHER"])) * 12,
...
ctx["PFLEGEVERSICHERUNG_BASE_RATE_MAX_AGE"] = int(_v["PFLEGEVERSICHERUNG_BASE_RATE_MAX_AGE"])  # § 55 Abs. 1 SGB XI
...
ctx["PFLEGEVERSICHERUNG_DISCOUNT_MIN_CHILDREN"] = int(_v["PFLEGEVERSICHERUNG_DISCOUNT_MIN_CHILDREN"])
ctx["PFLEGEVERSICHERUNG_DISCOUNT_MAX_CHILDREN"] = int(_v["PFLEGEVERSICHERUNG_DISCOUNT_MAX_CHILDREN"])
...
ctx["PENSIONREFUNDGERMANY_MAX_FEE"] = int(_v["PENSIONREFUNDGERMANY_MAX_FEE"])  # €
ctx["GERMANYPENSIONREFUND_MAX_FEE"] = int(_v["GERMANYPENSIONREFUND_MAX_FEE"])  # €
...
ctx["BVG_FINE"] = fail_on("2026-12-31", int(_v["BVG_FINE"]))
ctx["BVG_REDUCED_FINE"] = fail_on("2026-12-31", int(_v["BVG_REDUCED_FINE"]))
ctx["DEUTSCHLAND_TICKET_PRICE"] = fail_on("2026-12-31", int(_v["DEUTSCHLAND_TICKET_PRICE"]))
...
ctx["SCHENGEN_VISA_FEE"] = int(_v["SCHENGEN_VISA_FEE"])
ctx["NATIONAL_VISA_FEE"] = int(_v["NATIONAL_VISA_FEE"])
ctx["NATIONAL_VISA_RENEWAL_FEE"] = int(_v["NATIONAL_VISA_RENEWAL_FEE"])
ctx["RESIDENCE_PERMIT_REPLACEMENT_FEE"] = int(
    _v["RESIDENCE_PERMIT_REPLACEMENT_FEE"]
)  # After a passport change (€) - § 45c AufenthG
ctx["MIN_PERMANENT_RESIDENCE_FEE"] = int(_v["MIN_PERMANENT_RESIDENCE_FEE"])  # For Turkish citizens
ctx["MAX_PERMANENT_RESIDENCE_FEE"] = int(_v["MAX_PERMANENT_RESIDENCE_FEE"])  # § 44 AufenthG
ctx["FAST_TRACK_FEE"] = int(_v["FAST_TRACK_FEE"])  # § 47 AufenthG
...
ctx["FREELANCE_VISA_MIN_INCOME"] = fail_on("2026-12-31", int(_v["FREELANCE_VISA_MIN_INCOME"]))
...
ctx["CITIZENSHIP_TEST_FEE"] = fail_on("2026-12-31", int(_v["CITIZENSHIP_TEST_FEE"]))
...
ctx["ERWEITERTE_MELDEBESCHEINIGUNG_FEE"] = fail_on(
    "2026-12-31", int(_v["ERWEITERTE_MELDEBESCHEINIGUNG_FEE"])
)  # (€) - service.berlin.de/dienstleistung/120702
ctx["GEWERBEANMELDUNG_FEE"] = fail_on(
    "2026-12-31", int(_v["GEWERBEANMELDUNG_FEE"])
...
ctx["HUNDESTEUER_FIRST_DOG"] = fail_on("2026-12-31", int(_v["HUNDESTEUER_FIRST_DOG"]))  # §4 HuStG BE, (€/y)
ctx["HUNDESTEUER_MORE_DOGS"] = fail_on("2026-12-31", int(_v["HUNDESTEUER_MORE_DOGS"]))  # §4 HuStG BE, (€/y)
...
ctx["KSK_MIN_INCOME"] = fail_on("2026-12-31", int(_v["KSK_MIN_INCOME"]))  # (€/y) - §3 Abs. 1 KSVG
...
ctx["KSK_MIN_HEALTH_INSURANCE_INCOME"] = fail_on(
    "2026-12-31", int(_v["KSK_MIN_HEALTH_INSURANCE_INCOME"])
)  # Mindestbeitragsberechnungsgrundlage (€/y)

ctx["ORDNUNGSAMT_DANGEROUS_DOG_FEE"] = fail_on(
    "2026-12-31", int(_v["ORDNUNGSAMT_DANGEROUS_DOG_FEE"])
...
ctx["LICENSE_PLATE_COST"] = fail_on("2027-12-31", int(_v["LICENSE_PLATE_COST"]))  # Cost of making license plates

ctx["FIRST_AID_COURSE_COST"] = fail_on(
    "2027-12-31", int(_v["FIRST_AID_COURSE_COST"])
...
ctx["LEGAL_HOTLINE_COST_PER_MINUTE"] = fail_on(
    "2026-03-20", int(_v["LEGAL_HOTLINE_COST_PER_MINUTE"])
